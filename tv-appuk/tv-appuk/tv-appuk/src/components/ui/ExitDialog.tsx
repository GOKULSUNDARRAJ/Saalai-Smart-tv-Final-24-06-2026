...
export function ExitDialog() {
  ...
  const { focusKey, ref, setFocus, getCurrentFocusKey } = useFocusable({ focusKey: 'exit-dialog', isFocusBoundary: true, trackChildren: true })
  const prevFocusKeyRef = useRef<string>('nav-home')

  useEffect(() => {
    if (isExitDialogOpen) {
      prevFocusKeyRef.current = getCurrentFocusKey() ?? 'nav-home'
  ...
  const handleCancel = () => {
    closeExitDialog()
    setTimeout(() => setFocus(prevFocusKeyRef.current || 'nav-home'), 80)
  }
