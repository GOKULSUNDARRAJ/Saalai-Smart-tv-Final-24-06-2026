...
import { playNative, playNativeMovie, shouldUseNativePlayer } from '../platform/nativeVideoPlayer'
          ...
          <div className="relative z-10 px-safe" style={{ paddingTop: 48, paddingBottom: 80, display: 'flex', flexDirection: 'column', maxWidth: 768 }}>
            ...
            <div style={{ display: 'flex', marginTop: 16 }}>
              <FocusableButton
                ...
                onEnterPress={async () => {
                  const launched = item.movieId
                    ? await playNativeMovie(item.streamUrl, item.title, item.movieId, item.startPositionMs ?? 0)
                    : await playNative(item.streamUrl, item.title, item.startPositionMs)
