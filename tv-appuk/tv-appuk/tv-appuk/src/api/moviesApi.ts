...
export async function fetchMovieDashboard(): Promise<MovieDashboard> {
  ...
  try {
    ...
    const nonBanner = list.filter((c) => {
      if (c.categoryId === 0) return false
      const isContinue = c.categoryName.toLowerCase().includes('continue')
      if (isContinue && (!c.channels || c.channels.length === 0)) return false
      return true
    })
    ...
    for (const c of nonBanner) {
      channelsByCategory[c.categoryId] = c.channels.map((ch) => {
        const channel = ch as { channelId: number; channelName: string; channelLogo: string; channelDuration?: string; playedDuration?: string }
        return {
          id: channel.channelId,
          name: channel.channelName,
          logo: channel.channelLogo,
          channelDuration: channel.channelDuration || undefined,
          playedDuration: channel.playedDuration || undefined,
        }
      })
    }
...
export async function fetchMovieDetails(channelId: number): Promise<MovieDetail | null> {
  ...
  try {
    ...
    if (checkAccessDenied(d)) return null
