...
export function PlayerScreen() {
  ...
  useEffect(() => {
    ...
    fetchMovieDetails(movieId).then(d => {
      if (d && d.related.length > 0) setRelatedMovies(d.related.slice(0, 6))
    })
  ...
  useEffect(() => {
    mltCardRefs.current[mltFocusIdx]?.scrollIntoView({ block: 'nearest', behavior: 'smooth' })
  }, [mltFocusIdx])
  ...
  const clearNextUpTimer = useCallback(() => {
    if (nextUpTimerRef.current !== null) { clearInterval(nextUpTimerRef.current); nextUpTimerRef.current = null }
  }, [])
  ...
  const playNext = useCallback(() => {
    ...
    if (!cur?.playlist || cur.playlistIndex == null) { goBack(); return }
    const nextIdx = cur.playlistIndex + 1
    const nextItem = cur.playlist[nextIdx]
    if (!nextItem) { goBack(); return }
    setIsBuffering(true)
    setIsPlaying(false)
    setCurrentTime(0)
    setDuration(0)
    setError(null)
    setLocalContent({ ...cur, streamUrl: nextItem.url, title: nextItem.title, thumbnailUrl: nextItem.thumbnailUrl ?? '', playlistIndex: nextIdx, startPositionMs: 0, movieId: undefined })
  ...
  const cancelNextRef = useRef(cancelNext)
  useEffect(() => { cancelNextRef.current = cancelNext }, [cancelNext])
  ...
  useEffect(() => {
    ...
    const onEnded = () => {
      const cur = localContentRef.current
      if (cur?.playlist && cur.playlistIndex != null && cur.playlistIndex + 1 < cur.playlist.length) {
        showNextUpRef.current = true
        setShowNextUp(true)
        setNextUpCountdown(10)
        let count = 10
        if (nextUpTimerRef.current !== null) clearInterval(nextUpTimerRef.current)
        nextUpTimerRef.current = window.setInterval(() => {
          count--
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (showNextUpRef.current) {
        ...
        if (key === TVKey.OK || key === TVKey.PLAY) { playNextRef.current(); return }
        if (key === TVKey.BACK) { cancelNextRef.current(); return }
        return
      }
      ...
      if (showMoreLikeThis) {
        ...
        if (key === TVKey.BACK) {
          ...
          setActiveFocus('seekbar')
          return
        }
        if (key === TVKey.LEFT) {
          if (mltFocusIdx % 2 === 0) { setShowMoreLikeThis(false); setActiveFocus('seekbar') }
  ...
  }, [goBack, revealControls, hideControlsTimer, showControls, seekVideo, activeFocus, showMoreLikeThis, mltFocusIdx, relatedMovies, navigateToMovieDetail])
  ...
  return (
    <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, background: '#000', overflow: 'hidden', display: 'flex', flexDirection: 'row' }}>
      <div
        style={{ position: 'relative', flex: 1, overflow: 'hidden', transition: 'flex 0.35s ease' }}
      ...
      {/* Next Up overlay */}
      {showNextUp && localContent?.playlist && localContent.playlistIndex != null && localContent.playlistIndex + 1 < localContent.playlist.length && (
        <div style={{
          ...
          <div style={{ color: 'rgba(255,255,255,0.55)', fontSize: 11, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 4 }}>Up Next</div>
          <div style={{ color: '#fff', fontSize: 18, fontWeight: 700, marginBottom: 4 }}>Playing in {nextUpCountdown}s</div>
          <div style={{ color: 'rgba(255,255,255,0.75)', fontSize: 13, marginBottom: 14, maxWidth: 260, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {localContent.playlist[localContent.playlistIndex + 1].title}
          </div>
          <div style={{ display: 'flex', gap: 10 }}>
            <button
              onClick={playNext}
              style={{ background: '#e50914', color: '#fff', border: 'none', borderRadius: 6, padding: '8px 16px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}
            >
              Play Now
            </button>
            <button
              onClick={cancelNext}
              style={{ background: 'rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.8)', border: 'none', borderRadius: 6, padding: '8px 16px', fontSize: 13, cursor: 'pointer' }}
            >
              Cancel
            </button>
          </div>
