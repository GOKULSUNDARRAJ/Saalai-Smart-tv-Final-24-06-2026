...
import type { ContentItem, ContentRow as ContentRowType, PlaylistItem } from '../types/content'
...
export function HomeScreen() {
    ...
    for (const row of rowsRef.current) {
      ...
      if (idx >= 0) {
        ...
        if (playable.length > 1) {
          playlist = playable.map(i => ({ url: i.streamUrl, title: i.title, thumbnailUrl: i.thumbnailUrl }))
          playlistIndex = playable.findIndex(i => i.id === item.id)
        }
        break
      }
    }
    const itemWithPlaylist: ContentItem = { ...item, playlist, playlistIndex }
    const launched = await playNative(item.streamUrl, item.title, playlist, playlistIndex)
    if (!launched) navigate('player', itemWithPlaylist)
  ...
  const handleItemFocused = useCallback((item: ContentItem) => {
    for (const row of rowsRef.current) {
      const idx = row.items.findIndex(i => i.id === item.id)
      if (idx >= 0) {
        lastFocusKeyRef.current = `card-row-${row.id}-${idx}`
        break
      }
    }
  }, [])
  ...
  return (
    <FocusContext.Provider value={focusKey}>
      <div ref={ref} style={{ height: '100%', overflow: 'hidden', backgroundImage: `url(${BANNERS[bannerIdx].img})`, backgroundSize: 'cover', backgroundPosition: 'center', position: 'relative', transition: 'background-image 0.8s ease-in-out' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.65)', zIndex: 0, pointerEvents: 'none' }} />
        ...
        <div style={{
          position: 'absolute', right: '5vw', top: 265, zIndex: 2, pointerEvents: 'none',
