...
function notifySubscribeRequired() {
  useAppStore.getState().openSubscribePopup()
}
...
export async function fetchTvShowDashboard(): Promise<TvShowDashboard> {
  ...
  try {
    const res = await fetch('https://staging.saalai.tv/saalai_app/secure/getTvShowDashboadList', {
      method: 'POST',
...
export async function fetchTvShowList(categoryId: number, offset = 0, count = 100): Promise<TvShowItem[]> {
  ...
  try {
    ...
    const res = await fetch('https://staging.saalai.tv/saalai_app/secure/getTvShowList', {
      method: 'POST',
      headers: {
        Authorization: token,
...
export async function fetchTvShowEpisodeList(
  ...
  try {
    ...
    const res = await fetch('https://staging.saalai.tv/saalai_app/secure/getTvShowEpisodeList', {
      method: 'POST',
      headers: {
        Authorization: token,
        'Content-Type': 'application/x-www-form-urlencoded',
    ...
    const text = await res.text()
    const data = typeof text === 'string' ? JSON.parse(text) : text
    if (data.error_type === '201') { notifySubscribeRequired(); return empty }
    if (!data.status) return empty
    const r = data.response ?? {}
    const ed = r.episodeDetails ?? null
    const list: EpisodeItem[] = (r.tvShowEpisodeList ?? []).map((e: {
      episodeId: number; episodeName: string; episodeLogo: string; episodeDate: string; channelId: string
    }) => ({
