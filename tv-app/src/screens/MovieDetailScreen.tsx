...
import { hhmmssToMs } from '../api/moviesApi'
...
export function MovieDetailScreen() {
  ...
  useEffect(() => {
    ...
    fetchMovieDetails(selectedMovieId).then((d) => {
      setDetail(d)
      setLoading(false)
      setTimeout(() => setFocus('moviedetail-play'), 100)
    })
  ...
  }, [detail, navigate, refreshDetail])

  const handleResume = useCallback(async () => {
    if (!detail) return
    const startMs = hhmmssToMs(detail.playedTime)
              ...
              <div style={{ flex: 1, paddingTop: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 16 }}>
                  <ActionButton
                    ...
                    onArrow={(dir) => {
                      if (dir === 'right') {
                        setFocus(detail.playedTime && hhmmssToMs(detail.playedTime) > 0 ? 'moviedetail-resume' : 'moviedetail-back')
                        return false
                      }
                      if (dir === 'down') {
                  ...
                  {detail.playedTime && hhmmssToMs(detail.playedTime) > 0 ? (
                    <>
                      <ActionButton
